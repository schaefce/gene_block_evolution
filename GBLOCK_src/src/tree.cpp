...
Tree::Tree(std::pair<Tree*,double> left, std::pair<Tree*,double> right, std::string name) {
  this->left = left;
  this->right = right;
  this->name = name;
}
...
Tree* parse_subtree(const std::string& s, int& p) {
  ...
  if (t == "(") {
    ...
    return new Tree(std::make_pair(left, left_weight), std::make_pair(right, right_weight), name);
  }
